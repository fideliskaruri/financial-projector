      ...
      <CardContent className="flex items-center gap-4 p-5 sm:p-6">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div>
            ...
            <p className="text-2xl font-semibold tabular-nums">{formatKES(spent)}</p>
          </div>
          <div className="space-y-1 text-sm">
            <div className="flex items-center justify-between gap-3">
              <span className="text-muted-foreground">Budgeted</span>
              <span className="font-medium tabular-nums">{formatKES(budgeted)}</span>
            </div>
            <div className="flex items-center justify-between gap-3">
              <span className="text-muted-foreground">Remaining</span>
              <span className={cn("font-medium tabular-nums", remaining >= 0 ? "text-emerald-500" : "text-destructive")}>{formatKES(remaining)}</span>
            </div>
          </div>
        </div>
      </CardContent>
